...
없으면 보수적으로 `LARGE_MISSING`을 사용합니다. 여러 유형이 탐지되면 타입 가중치
`CRACK(3.0) > WEAR(2.0) > MISSING(1.0)`에 confidence와 파손 비율을 작은 보정값으로 더해
대표 유형을 선정합니다. 이 가중치는 대표 유형 선정에만 사용하며 `damage_score`와
`repair_priority`에는 영향을 주지 않습니다. 전체 유형별 탐지 결과는
