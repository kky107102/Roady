...
우리 프로젝트는 카메라로 점자블록 파손을 찾아야 해서 비전 AI가 핵심이다. 처음에는 YOLO가 물체를 네모 박스로 찾는 모델이라는 정도만 알고 있었다. 그런데 받은 AI Hub 라벨을 확인해 보니 점자블록 데이터가 polygon으로 되어 있어서 Detection과 Segmentation 중 무엇을 써야 할지부터 공부할 필요가 있었다. 또 최종적으로 Jetson Orin Nano에서 돌려야 하므로 정확도만 볼 수 없고 속도와 메모리도 같이 봐야 했다.

이번에 중점적으로 찾아본 것은 다음과 같다.
...
우리 프로젝트에서는 단일 프레임의 threshold만으로 판단하지 않고 다음을 함께 사용하려고 한다.
...
추가로 받고 있는 `인도보행 영상` 데이터는 XML 형식이며 Surface Masking에 `braille_guide_blocks`의 `normal/damaged` polygon이 들어 있다. 따라서 첫 데이터의 JSON과 두 번째 데이터의 XML을 각각 읽은 뒤, 바로 YOLO 파일로 만들기 전에 같은 내부 라벨 구조로 맞추는 과정이 필요하다. 두 데이터의 `damaged` 기준이 같은지도 샘플 mask를 그려 보고 확인할 예정이다.
